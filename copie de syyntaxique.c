...
void programme(void) {
  ...
  //printf("Analayse terminé avec succès .... Merci \n");
...
void declFonction(void){
  ...
  //affiche_dico();
  //printf("nom fonction %s et le id est %d\n", yytext, rechExecutable);
	  ...
	  switch (classe){
		  ...
		  case ID_VAR : {
			...
			if (classe == '['){
			  ...
			  classe = yylex();
			  if (dico.tab[rechIdentif].type != T_TABLEAU){
				printf("le type est %d et la classe est %d\n", dico.tab[rechIdentif].type, classe);
